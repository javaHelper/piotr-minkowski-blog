...
public class TransactionsListener {
    ...
    @KafkaListener(id = "transactions", topics = "transactions", groupId = "a", concurrency = "3")
    ...
    private void process(Order order) {
        Account accountSource = repository.findById(order.getSourceAccountId()).orElseThrow();
        Account accountTarget = repository.findById(order.getTargetAccountId()).orElseThrow();
